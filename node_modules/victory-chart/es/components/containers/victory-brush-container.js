  ...
  return _temp = _class = function (_base) {
    ...
    _createClass(VictoryBrushContainer, [{
      ...
      value: function getSelectBox(props, coordinates) {
        var x = coordinates.x,
            y = coordinates.y;
        var selectionStyle = props.selectionStyle,
        ...
        return x[0] !== x[1] && y[0] !== y[1] ? React.cloneElement(selectionComponent, {
          width: Math.abs(x[1] - x[0]) || 1,
          height: Math.abs(y[1] - y[0]) || 1,
          x: Math.min(x[0], x[1]),
          y: Math.min(y[0], y[1]),
          cursor: "move",
          style: _defaults({}, selectionComponentStyle, selectionStyle)
        }) : null;
      }
    }, {
      key: "getHandles",
      value: function getHandles(props, coordinates) {
        ...
        var handleProps = {
          top: dimension !== "x" && _assign({ x: x[0], y: y[1] - handleWidth / 2 }, yProps),
          bottom: dimension !== "x" && _assign({ x: x[0], y: y[0] - handleWidth / 2 }, yProps),
          left: dimension !== "y" && _assign({ y: y[1], x: x[0] - handleWidth / 2 }, xProps),
          right: dimension !== "y" && _assign({ y: y[1], x: x[1] - handleWidth / 2 }, xProps)
        };
      ...
      value: function getRect(props) {
        var currentDomain = props.currentDomain,
            cachedSelectedDomain = props.cachedSelectedDomain;

        var selectedDomain = _defaults({}, props.selectedDomain, props.domain);
        var domain = _isEqual(selectedDomain, cachedSelectedDomain) ? _defaults({}, currentDomain, selectedDomain) : selectedDomain;
    ...
    eventHandlers: {
      ...
      onMouseMove: function (evt, targetProps) {
        var mutations = BrushHelpers.onMouseMove(evt, targetProps);
